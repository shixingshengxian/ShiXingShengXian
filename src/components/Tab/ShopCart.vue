<template>
  <div class="cart">
    <div class="cart-layout">
      <div class="cart-card" v-show="goods.length">
        <div class="flex-box">
      ...
      <div class="cart-none" v-show="!goods.length">
        <img
    ...
    <div class="fresh-row cart-bottom-wrapper" v-show="goods.length">
    ...
    <div class="cart-actions" @click="edit_mode()" v-show="goods.length">
...
export default Vue.extend({
  ...
  methods: {
    check_all() {
      if (
        this.check_goods.length >= 0 &&
        this.check_goods.length != this.goods.length
      ) {
        this.check_goods = this.goods;
        // } else if ((this.check_goods.length = 0)) {
        //   this.goods.forEach(item => {
        //     this.check_goods.push(item);
        //   });
      } else if ((this.check_goods.length = this.goods.length)) {
        this.check_goods = [];
      } else {
        this.goods.forEach(item => {
          this.check_goods.push(item);
    ...
    del() {
      let arr = [];
      this.goods.forEach(item => {
        if (this.check_goods.indexOf(item) >= 0) {
          // console.log(item);
        } else {
          arr.push(item);
        }
